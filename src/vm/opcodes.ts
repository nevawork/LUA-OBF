...
export enum Op {
  ...
  DECL = 2,      // A        append A fresh cells (locals entering scope)
  ...
  SETLIST = 17,  // A        append top A values (A==-1: whole multret range) to table under them
  CLOSURE = 18,  // A        push closure(protos[A])
  ...
  ADJUST = 43,   // A         force exactly A values on top; clear mr
  ADJUST_ONE = 44, //        force exactly 1 value on top; clear mr
  MSET = 45,     // A        A targets: layout [k,t] xA then A values; env-marker store
  FORPREP = 46,  // A,B      A=control slot, B=rel exit; pops [start, limit] or [start, limit, step]
  FORLOOP = 47,  // A,B      A=control slot, B=rel back to body
  GFORPREP = 48, // A,B      A=#ctrl slots, B=rel exit; pops f,s,ctrl; iterates once
  GFORLOOP = 49, // A,B      A=#ctrl slots, B=rel back to body
  ESCAPE = 50,   // A,B      reserved deopt valve: native escape[A]
}
