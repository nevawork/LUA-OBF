...
export interface Token {
  type: Tok;
  value: string; // decoded string content for String, raw text otherwise
  line: number;
  col: number;
}
...
export function lex(src: string, version: LuaVersion = "lua51"): Token[] {
  ...
  const push = (type: Tok, value: string, l = line, c = col) =>
  ...
  while (i < src.length) {
    ...
    if (ch === "-" && src[i + 1] === "-") {
      ...
      if (src[i] === "[" && (src[i + 1] === "[" || src[i + 1] === "=")) {
        ...
        if (long !== null) continue; // long comment consumed
  ...
  function readLongBracket(): string | null {
    ...
    if (endIdx === -1) throw new LuaSyntaxError("unterminated long bracket", line, col);
  ...
  function readQuoted(quote: string): string {
    ...
    let out = "";
    for (;;) {
      if (i >= src.length) throw new LuaSyntaxError("unterminated string", line, col);
      const c = src[i];
      if (c === quote) {
        adv();
        return out;
      ...
      if (c === "\\") {
        adv();
        const e = src[i];
        if (e === undefined) throw new LuaSyntaxError("unterminated string escape", line, col);
        if (e in ESCAPES) {
          out += ESCAPES[e];
          adv();
        } else if (/[0-9]/.test(e)) {
