...
exports.encryptStrings = encryptStrings;
exports.flattenControlFlow = flattenControlFlow;
exports.injectOpaqueJunk = injectOpaqueJunk;
const parser_1 = require("../lang/parser");
function resetCounter() {
    counter = 0;
}
let counter = 0;
const uid = () => `${(counter++).toString(36)}xq`;
...
// ------------------------------------------------------------------
function encryptStrings(chunk) {
    const fnName = `nk${uid()}`;
    let count = 0;
    const encOf = (value, key) => {
    ...
    const rewriteExpr = (e) => {
        switch (e.kind) {
            ...
            case "Func":
                ...
                return e;
            case "Table":
                e.fields = e.fields.map((f) => f.kind === "Keyed"
    ...
    const rewriteBlock = (b) => {
        for (const s of b.stats)
            rewriteStat(s);
        if (b.ret)
            b.ret.exprs = b.ret.exprs.map(rewriteExpr);
    };
    const rewriteStat = (s) => {
        switch (s.kind) {
            case "LocalDecl":
            ...
            case "CallStat": {
                ...
                if (r.kind === "Call" || r.kind === "MethodCall")
                    s.call = r;
                break;
            }
            case "Do":
    ...
    const decChunk = (0, parser_1.parse)(`
...
const isFlattenable = (s) => s.kind === "Assign" || s.kind === "LocalDecl" || s.kind === "ExprStat" ||
 ...
 */
function flattenControlFlow(block, ctx) {
    ...
    const process = (b) => {
        ...
        for (const s of b.stats) {
            ...
            else if (s.kind === "NumFor")
                process(s.body);
            else if (s.kind === "GenFor")
                process(s.body);
            else if (s.kind === "Do")
                process(s.body);
            else if (s.kind === "FuncStat")
                process(s.func.body);
...
function buildMachine(run, nextKey) {
    ...
    for (let idx = 0; idx < run.length; idx++) {
        ...
        if (s.kind === "LocalDecl") {
            ...
            if (s.exprs.length > 0) {
                bodyStats.push({
                    kind: "Assign",
                    targets: s.names.map((n) => ({ kind: "Name", name: n })),
                    exprs: s.exprs,
                });
            }
        }
    ...
    bodyStats.forEach((s, idx) => {
        const body = { stats: [s], ret: undefined };
            ...
            stats: [
                {
                    ...
                    exprs: [
                        ...
                        ...hoisted.map(() => ({ kind: "Nil" })),
                        ...(decoys > 0 ? [{ kind: "Number", value: 7, raw: "" }] : []),
                        ...(decoys > 1 ? [{ kind: "Number", value: 11, raw: "" }] : []),
                    ],
                },
                {
                    kind: "While",
...
function injectOpaqueJunk(block, density = 0.12, rng) {
    const roll = () => (rng ? rng.int(10000) / 10000 : Math.random());
    const recurse = (b) => {
        const out = [];
        for (const s of b.stats) {
        ...
        b.stats = out;
        if (b.ret && roll() < density)
            b.stats.push(makeJunk());
    };
    recurse(block);
    function makeJunk() {
        const a = `jz${uid()}`;
