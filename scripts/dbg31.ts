...
(async () => {
  ...
  for (let i = 0; i < flat.length; i++) {
    const p = flat[i];
    console.log(`flat[${i}]: params=${p.params}, isVararg=${p.isVararg}, upvals=${p.upvals.length}, numSlots=${p.numSlots}, consts=${p.consts.length}, code=${p.code.length}`);
    console.log(`  consts:`, p.consts.map(c => typeof c === 'string' ? `"${c.slice(0,30)}"` : c).slice(0, 20));
  }
})();
